...
function ProductCard({ name, tagline, desc, icon, color, badge, tags }) {
  const [hov, setHov] = useState(false);
  return (
    <div onMouseEnter={() => setHov(true)} onMouseLeave={() => setHov(false)} style={{ background: hov ? T.bgCardHov : T.bgCard, border: `1px solid ${hov ? color + "88" : T.border}`, borderRadius: 20, padding: 32, transition: "all 0.3s", boxShadow: hov ? `0 0 40px ${color}22, 0 12px 40px #000a` : "0 4px 24px #0006", position: "relative", overflow: "hidden", height: "100%" }}>
...
export default function Products() {
  const products = [
    { name: "Scifi Codes", tagline: "Digital Pin & Gift Card Store", desc: "Your one-stop destination for digital game codes, gift cards, prepaid pins, and software licenses.", icon: "🎮", color: T.cyan, badge: "Live", tags: ["Game Codes", "Gift Cards"] },
    { name: "Scifi Data", tagline: "Data Plans & Connectivity", desc: "Affordable internet data bundles and mobile top-ups for the African market.", icon: "📡", color: T.violet, badge: "Live", tags: ["Data Bundles", "eSIM", "Reselling API"] }
  ];
